...
**WARNING!** Always keep in mind that using these settings, you could lose connection! Always have a backup way to reach the system.
...
These settings configure network interfaces to provide desired ip, prefix, gateway, etc.
These settings also covers routes definitions on interfaces and other advanced settings.

These settings rely on nmcli (NetworkManager) module from community.general collection will be used to configure network. These settings should provide all features available in the main nmcli module.
...
While we try to bind to nmcli module options for convention, some of these keys provide more integrated features:
...
* `ip4`: can be set using a simple ipv4, then this will use
...
* `ip4_manual`: allows to pass additional list of ip/prefix.
...
* `gw4`: has higher precedence over `networks[item.network]['gateway4']`
  ...
  if set. Note that gw4 cannot be set at the same time as `never_default4`
  (mutually exclusive).
...
By default, if interfaces are down, these settings will have them up, and at the same 
