...
Hosts are defined by default at root level of the inventory.
For convenience, we will store the hosts files and group files inside a dedicated ``cluster/`` folder.

Hosts are always part of 3 groups:
...
Hosts must always be under the ``all`` and ``hosts`` keys inside their YAML files.

By default, we store all hosts into the same file, ``cluster/hosts.yml``, but nothing prevents from splitting this file into smaller ones.
...
Then you can add hosts as dict keys under hosts, using their hostname.
...
Under host hostname, it is possible to define network interfaces, as a list inside ``network_interfaces`` key.
...
If you plan to deploy the host via this interface over the network (PXE) you will also need to set a way to identify it, like its mac.
...
Each host needs to be in at least 3 specific groups:
...
You can then also add hosts into custom groups for ease, as long as they do not use the 3 reserved prefixes.

Function group
--------------

Function groups define the purpose of the hosts.
Most of the time, these groups are also used to create the associated Ansible playbooks, to define what to configure on hosts of the same function group.

Function groups are always prefixed by ``fn_``.

To add a host in a function group, edit/create file ``cluster/fn`` (not ``cluster/fn.yml`` as this is not a YAML format here, but INI).
Each group is between ``[ ]`` and hosts inside this group are just listed bellow.
...
.. note::
  ...
  Protip: you can use ranges to define hosts in group files. In this example, the 4 c00X hosts can also be written ``c00[1:4]``.
...
OS groups define the OS related settings of the associated hosts.

OS groups are always prefixed by ``os_``.

To add a host in an OS group, edit/create file ``cluster/os``, like for function groups.
...
Hardware groups define the hardware related settings of the associated hosts.

Hardware groups are always prefixed by ``hw_``.

To add a host in an hardware group, edit/create file ``cluster/hw``, like for function groups.
...
Generate range of hosts
=======================

Since hosts are based on YAML files, it is easy to generate them using a bash script.
...
Save, make this script executable, and run it asking for 4 hosts:
...
Just add the alias as a list of alias under the host name, this will result in an entry that will be linked to the host hostname.
...
Pinging foobar will end up on 10.10.0.1, like pinging mgt1.

Per nic alias
-------------

You can add the alias inside a nic interface dict, this will result in an entry that will be linked to the host nic.
...
Pinging foobar will end up on 10.20.0.1, like pinging mgt1-ib0.
