...
.. warning::
  ...
  Prometheus is a **MONITORING** tool, not a **SUPERVISION** tool. As so, it is really great to gather metrics data (integers, floats) and
  ...
  Consider using a tool like Icinga2 or equivalent for supervision (I might add a role to handle Icinga2 when I can).
  Example: What is the current load of my cluster ? -> monitoring. Is server c001 answering ping and is ssh able ? -> supervision.
