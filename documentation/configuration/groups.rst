...
Since the stack is based on Ansible, it is possible to create custom logical groups of hosts to organize your inventory.
...
- ``fn_.*``: these groups are function groups. Variables stored in these groups should start with the specific ``fn_`` prefix.
    - ``fn_management``: this group should contain all manager/controller hosts.
