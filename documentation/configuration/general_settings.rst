...
Create/update file ``group_vars/all/dns.yml`` with the following content:

.. code-block:: yaml

  bb_domain_name: bluebanquise-cluster.local

Create/update file ``group_vars/all/time.yml`` with the following content:
...
These variables are mainly used for High Availability purposes. You can ignore them if not needed.
